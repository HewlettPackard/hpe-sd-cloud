...
* [HPE Service Director Helm chart Deployment](#hpe-service-director-helm-chart-deployment)
  ...
  * [Prerequisites](#prerequisites)
    ...
    * [4. Kubernetes and Helm version](#4-kubernetes-and-helm-version)
  ...
  * [Configuring the network to access HPE SD MUSE](#configuring-the-network-to-access-hpe-sd-muse)
...
The subfolder [/chart](./) contains the Helm chart files that contain:
...
Consider, as an example, an instance of the `postgres` image in a Kubernetes pod. It is a clean PostgreSQL 13 image with an `sa` user ready for Service Director installation and, if having `muse` enabled, also a `muse` user is required (configure `muse_container.DB_NAME` and `muse_container.DB_USER`).

You can find the DB connection setup parameters in the `values.yaml` file. The parameters description can be found [here](#common-parameters). For details on creating your own DB password, see [this description](#dbsecret).

**NOTE**: If you are not using the Kubernetes [postgres-db](../../../templates/postgres-db/) deployment, you need to overwrite the db parameter values defined in [values](./values.yaml). They contain database-related environments and point to the installed database. Those values can be added to the deployment using the `-f` parameter in the `helm install` command.
...
Persistent storage is activated in all SD pods, if needed, with a `storageclass`. For the cases the `storageClass` can be modified, check the Helm chart [values](./values-production.yaml) file.
...
### 4. Kubernetes and Helm version

Only Kubernetes version 1.18.0 or later, Kubectl 1.18.0 or later and Helm 3 or later are supported.

**NOTE**: With an older Kubernetes version and Helm version, HPE SD components might not work as expected, or might not be able to deploy the Helm chart at all. Also note that if the [Helm Post Rendering](#helm-post-rendering) script will be used, Kubectl version 1.22.x or later is required.
...
- adjust the `SDCONF_activator_db_`-prefixed environment variables as appropriate for the [test values](./values.yaml) or [production values](./values-production.yaml)
- make sure that your database is ready to accept connections

**IMPORTANT:** The [values.yaml](./values.yaml) file defines the docker registry parameter for the used SD images. Specify this to point to the docker registry where your docker images are located (e.g.: `hub.docker.hpecorp.net/cms-sd`). For example,`- image: myrepository.com/sd-sp`.
...
helm repo add couchdb https://apache.github.io/couchdb-helm  (only if the sd_ui_uoc parameter is enabled)
...
```
helm repo add couchdb https://apache.github.io/couchdb-helm (only if sd_ui_uoc parameter is enabled)
helm repo add sd-chart-repo https://raw.githubusercontent.com/HewlettPackard/hpe-sd-cloud/master/kubernetes/helm/repo/
```
...
The following table lists common configurable chart parameters and their default values. See [values.yaml](./values.yaml) for all available options.
...
| `global.prometheus.enabled` | Set to `true` to deploy Prometheus with SD, see [Enabling and displaying metrics in Prometheus and Grafana](#enabling-and-displaying-metrics-in-prometheus-and-grafana).          | null    |
| `global.efk.enabled`        | Set to `true` to deploy ElasticSearch with SD, see [Displaying and analyzing SD logs in Elasticsearch and Kibana](#displaying-and-analyzing-sd-logs-in-elasticsearch-and-kibana). | null    |
...
| `muse.enabled`                                                 | Set it to `true` to enable HPE MUSE                                                                                                                                                                                                                                                                                                                                                                                                  | `true`                                                                                                                     |
| `sd_ui_uoc.enabled`                                            | Set it to `true` to enable the HPE UOC-based SD UI                                                                                                                                                                                                                                                                                                                                                                                   | `false`                                                                                                                    |
...
| `automountServiceAccountToken.enabled`                         | Set it to `true` to allow automounting the Service Account token in all SD pods.                                                                                                                                                                                                                                                                                                                                                     | `false`                                                                                                                    |
...
| `securityContext.readOnlyRootFilesystem`                       | Set Read-only Root Filesystem pods' Security Context                                                                                                                                                                                                                                                                                                                                                                                 | `false`                                                                                                                    |
| `securityContext.enableRunAsUser`                              | Set to false in OpenShift environments to allow running as random user                                                                                                                                                                                                                                                                                                                                                               | `true`                                                                                                                     |
| `securityContext.dropAllCapabilities`                          | Set to true to drop all linux kernel capabilities from containers                                                                                                                                                                                                                                                                                                                                                                    | `false`                                                                                                                    |
| `securityContext.addCapabilities`                              | Add specific linux capabilities to containers after dropping all                                                                                                                                                                                                                                                                                                                                                                     | `null`                                                                                                                     |
| `couchdb.enabled`                                              | Set it to `false` to disable CouchDB                                                                                                                                                                                                                                                                                                                                                                                                 | `true`                                                                                                                     |
| `redis.enabled`                                                | Set it to `false` to disable Redis                                                                                                                                                                                                                                                                                                                                                                                                   | `true`                                                                                                                     |
| `efk.enabled`                                                  | Set it to `true` to deploy ElasticSearch, FluentD and Kibana with SD, see [Displaying and analyzing SD logs in Elasticsearch and Kibana](#displaying-and-analyzing-sd-logs-in-elasticsearch-and-kibana)                                                                                                                                                                                                                              | `false`                                                                                                                    |
| `prometheus.enabled`                                           | Set it to `true` to deploy Prometheus and Grafana with SD, see [Enabling and displaying metrics in Prometheus and Grafana](#enabling-and-displaying-metrics-in-prometheus-and-grafana)                                                                                                                                                                                                                                               | `false`                                                                                                                    |
| **sdimage parameters**                                         |                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                            |
...
| `muse_container.env.DB_PORT`                                   | Port where the database is exposed                                                                                                                                                                                                                                                                                                                                                                                                   | `5432`                                                                                                                     |
| `muse_container.env.DB_NAME`                                   | Name of the database schema to use                                                                                                                                                                                                                                                                                                                                                                                                   | `muse`                                                                                                                     |
| `muse_container.env.DB_USER`                                   | Name of the user for the database                                                                                                                                                                                                                                                                                                                                                                                                    | `sa`                                                                                                                       |
| `muse_container.env.DB_PASSWORD`                               | Password of the user for the database                                                                                                                                                                                                                                                                                                                                                                                                | `secret`                                                                                                                   |
| **MUSE Shell parameters**                                      |                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                            |
| `muse_shell.env.AUTH_ENDPOINT`                                 | Endpoint (URL) of the HPE MUSE Authentication service or the URL of the load balancer in front of it                                                                                                                                                                                                                                                                                                                                 | `null`                                                                                                                     |
| `muse_shell.env.REGISTRY_ENDPOINT`                             | Endpoint (URL) of the HPE MUSE Registry service or the URL of the load balancer in front of it                                                                                                                                                                                                                                                                                                                                       | `null`                                                                                                                     |
| `muse_shell.env.NOTIFICATION_ENDPOINT`                         | Endpoint (URL) of the HPE MUSE Notification service or the URL of the load balancer in front of it                                                                                                                                                                                                                                                                                                                                   | `null`                                                                                                                     |
| `muse_shell.env.CONFIGURATION_ENDPOINT`                        | Endpoint (URL) of the HPE MUSE Configuration service or the URL of the load balancer in front of it                                                                                                                                                                                                                                                                                                                                  | `null`                                                                                                                     |
| **MUSE Auth parameters**                                       |                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                            |
| `muse_auth.env.LOG_LEVEL`                                      | Indicates the log level. Possible values are `trace`, `debug`, `info`, `warn`, `error`, `fatal`, `all`, and `off`                                                                                                                                                                                                                                                                                                                    | `info`                                                                                                                     |
| `muse_auth.env.JWT_SECRET_ACCESS`                              | Secure string to generate the normalized JWT access token                                                                                                                                                                                                                                                                                                                                                                            | `secret_access`                                                                                                            |
| `muse_auth.env.JWT_SECRET_REFRESH`                             | Secure string to generate the normalized JWT refresh token. The same value is recommended for all services                                                                                                                                                                                                                                                                                                                           | `secret_refresh`                                                                                                           |
| `muse_auth.env.JWT_TIMEOUT_ACCESS`                             | Timeout for the JWT access token. Use the following format: a number followed by s = seconds, m = minutes, or h = hours                                                                                                                                                                                                                                                                                                              | `1h`                                                                                                                       |
| `muse_auth.env.JWT_TIMEOUT_REFRESH`                            | Timeout for the JWT refresh token. Use the following format: a number followed by s = seconds, m = minutes, or h = hours                                                                                                                                                                                                                                                                                                             | `2h`                                                                                                                       |
| `muse_auth.env.SHELL_LOGIN_CALLBACK`                           | URL of the HPE MUSE Shell with the following format: `<shell_url>/auth/callback`                                                                                                                                                                                                                                                                                                                                                     | `http://localhost/auth/callback`                                                                                           |
| `muse_auth.env.SHELL_LOGOUT_CALLBACK`                          | URL of the HPE MUSE Shell with the following format: `<shell_url>`                                                                                                                                                                                                                                                                                                                                                                   | `http://localhost`                                                                                                         |
| `muse_auth.env.AUTH_TYPE`                                      | Determines the authentication method. Possible values are `local`, `saml`, and `oidc`                                                                                                                                                                                                                                                                                                                                                | `local`                                                                                                                    |
| **MUSE Auth parameters to configure OIDC**                     |                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                            |
| `muse_auth.env.OIDC_LOGIN_CALLBACK`                            | URL of the HPE MUSE Authentication service with the following format: `<auth_service_url>/oidc/login/callback`                                                                                                                                                                                                                                                                                                                       | `2h`                                                                                                                       |
| `muse_auth.env.OIDC_LOGOUT_CALLBACK`                           | URL of the HPE MUSE Authentication service with the following format: `<auth_service_url>/oidc/logout/callback`                                                                                                                                                                                                                                                                                                                      | `null`                                                                                                                     |
| `muse_auth.env.OIDC_ISSUER`                                    | Verifiable identifier for an issuer. An issuer identifier is a case sensitive URL using the HTTPS scheme, which contains a scheme, a host, and optionally, a port number, and path components, and no query or fragment components                                                                                                                                                                                                   | `null`                                                                                                                     |
...
| `muse_sd_ui.env.UI_PLUGIN_PORT`                                | Indicates the port of HPE SD UI Plugin                                                                                                                                                                                                                                                                                                                                                                                               | `null`                                                                                                                     |
| `muse_sd_ui.env.HPESD_PORT`                                    | Indicates the HPE SD UI Exposed port for HPE MUSE                                                                                                                                                                                                                                                                                                                                                                                    | `8080`                                                                                                                     |
...
| `muse_sd_ui_plugin.env.HPSA_PROTOCOL`                          | HPSA protocol                                                                                                                                                                                                                                                                                                                                                                                                                        | `http`                                                                                                                     |
| `muse_sd_ui_plugin.env.HPSA_TENANT`                            | Indicates tenant to be used in HPSA                                                                                                                                                                                                                                                                                                                                                                                                  | `UOC_SD`                                                                                                                   |
| `muse_sd_ui_plugin.env.HPSA_USER`                              | HPSA user                                                                                                                                                                                                                                                                                                                                                                                                                            | `admin`                                                                                                                    |
...
| Parameter                         | Description                                                                                                                                                                                                                                                                                                                                                                        | Default |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- |
| `prometheus.server_enabled` | If set to `false`, the Prometheus and Grafana pods will not be deployed. Use the values in this [config](./templates/prometheus/prometheus/configmap.yml) file to configure SD metrics to an alternative Prometheus server. Use these [dashboards](./templates/prometheus/grafana/)  to display SD metrics to an alternative Grafana server. | `true`  |
...
| `prometheus.grafana.enabled` | If set to `false`, the Grafana pod will not be deployed. Use these [dashboards](./templates/prometheus/grafana/) to display SD metrics to an alternative Grafana server. | `true` |
...
The SD-Healthcheck pod includes a Prometheus compilant endpoint with multiple available metrics that are exposed in a prebuilt Grafana dashboard. This metrics show the full chart component status depending on the selected filter labels (see [healthcheck pod section](#healthcheck-pod-for-service-director)). This endpoint and dashboard is available by specifying:

- `healthcheck.metrics.enabled=true`

You can see a preview of the prebuilt dashboard [here](/kubernetes/docs/Grafana.md/#sd-healthcheck-metrics).
...
The healthcheck pod comes as optional in SD Helm chart. You can deploy it using the parameter `healthcheck.enabled=true` during the `helm install` phase.
...
### Enable https for the healthcheck pod

The healthcheck pod does not enable HTTPS by default, it uses HTTP instead. This behavior is controlled by the `healthcheck.env.https_enabled` parameter, which can be found in the `values.yaml` file of the Helm chart.

To enable this feature, you must set the `healthcheck.env.https_enabled` parameter with the value `true` and provide a valid, publicly trusted SSL certificate with the correct domain name.

The certificate must be in the PKCS12 format and must be stored in the `./templates/healthcheck/secret.yaml` file of the HPE SD Helm chart.

Before saving the certificate to the secret file, you must encode it into the base64 format. The base64 encoding of the certificate is very sensitive and prone to errors, so it is recommended to use the following command:

```
cat ./keystore.p12  | base64  -w 0 > output.base64
```

where `keystore.p12` is the file where you have your certificate.
 
Once you get the base64 encoded file, paste the output into your secret file, under the `keystore.p12` parameter:
...
type: Opaque
data:
  keystore.p12: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
```       
...
| `healthcheck.env.log_level`                      | Used to set the log4j2 log level for the healthcheck pod, the possible levels are `OFF`, `FATAL`, `ERROR`, `WARN`, `INFO`, `DEBUG`, `TRACE`, `ALL`.                                                                              | `INFO`           |
| `healthcheck.env.https_enabled`                  | Used to activate secure https with the healthcheck pod, the possible values are `true`, `false`.                                                                                                                                 | `false`          |
...
A Helm post-renderer is simply an executable provided as a bash script that Helm will execute as post renderer. This script will apply some [Kustomize](https://kubernetes.io/docs/tasks/manage-kubernetes-objects/kustomization/) patches to `sd-helm-chart`, customizing HPE SD Helm's dependency charts without having to modify them directly by applying `Kustomize` customizations.

This file includes references to the files containing the patches Kustomize will apply. The following is an example of one of the patches:
...
The patch sets Kafka's StatefulSet `automountServiceAccountToken` to `false`.

The following table shows the changes the post-render script will apply:

| Dependency chart | Changes |
| :--- | :--- |
| `CouchDB` | (sd_ui_uoc parameter needs to be enabled) <br> • Add `automountServiceAccount: false` <br> • Add resources to `init-copy` container <br> • Enable `readOnlyRootFilesystem` <br> • Drop all linux capabilities from `couchDB` container <br> • Add needed linux capabilities <br> • Mount a configmap (`templates/ro-couchdb-initvolumes.yaml`) containing a script that does read-only filesystem setup tasks <br> |
| `Kafka (and Zookeeper)` | • Add `automountServiceAccount: false` |
| `Redis` | • Add `automountServiceAccount: false` |

#### Running The Post-Renderer

To run the post-render script with Helm, the post-renderer flag needs to be passed and set to the name of the executable, `kustomize-script` in this case. The script must be run from the **same directory** as the `kustomization.yaml` file. Complete the following steps, starting from a `sd-helm-chart.$TAG` tgz file:

**Note:** In order for the post-render script to work, at least Kubectl version 1.22.x or later is required.

1. Extract the `post-render` and the chart
```
$ tar -zvxf sd-helm-chart-$TAG
```
2. Run the following commands:

```
$ cd sd-helm-chart && cd post-render
$ helm install sd-cloud ../ --post-renderer=./kustomize-script.sh
```
The chart can also be downloaded from [HPE's Public Repository](https://github.com/HewlettPackard/hpe-sd-cloud/tree/master/kubernetes/helm/charts/sd-helm-chart).

**IMPORTANT:** Remember to apply the command each time you upgrade the release, otherwise the changes **will be lost**. As a recommendation, try to incorporate the Helm command in a pipeline or automated process so you do not forget to apply the post renderer when you install or upgrade.

For more information about post rendering in Helm, please see section *Post Rendering* in [the official Helm documentation](https://helm.sh/docs/topics/advanced/#post-rendering).

### Configuring the network to access HPE SD MUSE

Several HPE MUSE pods need to be accessed from the browser. This means that you need to give access to these pods outside the Kubernetes network.

The URLs to access these pods from the external network can be configured in the Helm chart's `values.yaml` file.

To expose the HPE MUSE pods to the external network, use one of the following options:

1. **NodePorts**
   
   This solution is suitable for testing environments only and is not recommended for production environments.
   
   The HPE MUSE pods will be exposed as `nodePorts` and several ports will be opened in the nodes of the Kubernetes cluster to access them. To do so, edit the `values.yaml` file in the HPE SD Helm chart and set a value in the `nodePort` parameter of the appropriate HPE MUSE pod.
   
   Some extra parameters also need to be defined in the `values.yaml` file on each HPE MUSE pod, setting the URL that will be used to connect to the pod. For example, to configure the MUSE authentication pod with a NodePort connection, specify the following data in the `values.yaml` file:
   
   ```
   muse_shell:
     env:
       AUTH_ENDPOINT: "http://160.34.5.123:31112/"
   ```   
   ...
   ```   
   where `160.34.5.123` is the IP address or hostname of some of the nodes of the Kubernetes cluster and `31112` is the nodePort opened in Kubernetes to receive request for the MUSE authentication pod.
   

2. **Ingress**

   This solution is suitable for both testing and production environments.

   With an Ingress, the HPE MUSE pods will be exposed through a single port opened in the Kubernetes cluster (the Ingress external port) and the Ingress rules will redirect the browser request to the right HPE MUSE pod.

   This solution will require the installation of an Ingress controller in the Kubernetes cluster (for example, NGINX Ingress or Kong ingress). For more information on how to choose the Ingress controller implementation that best fits your cluster, see section [Ingress Controllers](https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/) in the official Kubernetes documentation. 

   Some extra parameters also need to be defined in the `values.yaml` file on each HPE MUSE pod, setting the URL that will be used to connect to the pod.

   For example, to configure the MUSE authentication pod with an Ingress connection, specify the following data in the `values.yaml` file:
   
   ```
   muse_shell:
     env:
       AUTH_ENDPOINT: "http://160.34.5.123:31112/muse-auth"
      ```   
   ...
   where `160.34.5.123` is the IP address or hostname of some of the nodes of the Kubernetes cluster, `31112` is the Ingress port opened in Kubernetes to receive request for the MUSE authentication pod, and `/muse-auth` is the path the Ingress rules will use to redirect the requests to the MUSE authentication pod.
   
   You also need to configure the Ingress rules to redirect the requests to the HPE MUSE pods. The generic Ingress rules are as follows (the rules need to be tailored to your own Ingress configuration):
   ...
   ```   
     ...
     - host: sd.muse.com
       http:
         paths:
         - backend:
             serviceName: sd-helm-chart-sd-helm-sv-muse-auth
             servicePort: 4000
           path: /muse-auth
         - backend:
             serviceName: sd-helm-chart-sd-helm-sv-muse-configuration
             servicePort: 4003
           path: /muse-conf
         - backend:
             serviceName: sd-helm-chart-sd-helm-sv-muse-regitry-discover
             servicePort: 4001
           path: /muse-registry
         - backend:
             serviceName: sd-helm-chart-sd-helm-sv-muse-notification-service
             servicePort: 4002
           path: /muse-notif
         - backend:
             serviceName: sd-helm-chart-sd-helm-sv-muse-sd-ui-plugin
             servicePort: 3001
           path: /muse-uiplugin
         - backend:
             serviceName: sd-helm-chart-sd-helm-sv-muse-shell
             servicePort: 8080
           path: /
   
   ```   
      The `host` parameter is the IP address or hostname of the Kubernetes cluster and the `path` parameter must match the path of the URL defined for your HPE MUSE pods in the `values.yaml` file.
...
3. **Load Balancer**
   ...
   With a Load Balancer, the HPE MUSE pods can be reached directly without exposing additional pods in the Kubernetes cluster. Some rules need to be added to your Load Balancer to redirect the browser request to the right HPE MUSE pod.

   This solution will require the installation of a Load Balancer and a configured connection of the Load Balancer to the Kubernetes cluster. The Load Balancer must support routing rules based on the path of the incoming HTTP requests.

   Some extra parameters also need to be defined in the `values.yaml` file on each HPE MUSE pod, setting the URL that will be used to connect to the pod.

   For example, to configure the MUSE authentication pod with a Load Balancer connection, specify the following data in the `values.yaml` file:
   ...
   where `160.34.5.123` is the IP address or hostname of the Load Balancer, `31112` is the port opened in the Load Balancer to receive request for the MUSE authentication pod, and `/muse-auth` is the path the Load Balancer rules will use to redirect the requests to the MUSE authentication pod.
   
   You also need to configure the load Balancer rules to redirect the requests to the HPE MUSE pods. The generic Load Balancer rules are as follows (the rules need to be tailored to your own Load Balancer configuration):

   The following is an example of a NGINX Load Balancer configuration:
   
   ```   
   events {}
   http {
       upstream authentication {
           server 172.19.0.1:4000;
       }
   
       upstream configuration {
           server 172.19.0.1:4003;
       }
   
       upstream notification {
           ip_hash;
           server 172.19.0.1:4002;
       }
   
       upstream registry {
           server 172.19.0.1:4001;
       }
   
       upstream shell {
           server 172.19.0.1:4200;
       }
    ...
    #       add_header Access-Control-Allow-Origin *;
           ...
           # This block is only valid when shell is below the load balancer
           location / {
   ...
   where `172.19.0.1` is the Load balancer IP address assigned to the HPE MUSE pod's services.
